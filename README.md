...
| Replica | Drive id | Bytes | SHA256 (prefix) | Review / Math- pin |
|---|---|---:|---|---|
...
Each directory has a `SOURCE.json` custody record (Drive id, bytes, SHA256, visibility observation, mathematical identity when known). The matrix/lifetime parent (#63) is **Drive-primary**: it is not present on the Math- default branch, so this replica is the public byte-custody home for that exact proof file.
